...
io.on('connection', (socket) => {
  ...
  socket.on('join', ({ room_id, name }, cb) => {
    ...
    participants = []
    roomList.get(socket.room_id).getPeers().forEach(peer => participants.push(peer.name))
    socket.join(room_id)
    io.to(room_id).emit("newUser", participants)
    cb(roomList.get(room_id).toJson())
  })

  socket.on('getProducers', () => {
    if (!roomList.has(socket.room_id)) return
    //getPeers returns peers in the room object
    //peers is a map. so it finds the document with the socket.id and get the name from the doc
  ...
  socket.on('disconnect', () => {
    ...
    io.emit('userLeft', {name : `${roomList.get(socket.room_id) && roomList.get(socket.room_id).getPeers().get(socket.id).name}`})
