...
import { Mic, MicOff, PhoneMissed, Share, Video, VideoOff } from "react-feather"
import ShareOff from './assets/ShareOff.svg'
import Lobby from './Lobby';
import { RootState } from './redux/store';
import { setRoomId } from './features/formValues';
...
const Chat: React.FC<Props> = ({ socket }) => {
    ...
    getConsumeStream(producer_id).then(
      function ({ consumer, stream, kind }) {
        ...
        if (kind === 'video') {
          ...
          console.log(" Stream: ", stream.getVideoTracks().length)
          if (elem) {
  ...
  return (
    <>
      <Container>
        {!isJoined && <Lobby handleJoin={handleJoin} />}
        {
          isJoined && <Fragment>
            <div className='w-full h-full flex'>
              <div className='w-full h-full'>
                <Header className='w-full h-14 bg-white-500 grid grid-cols-3 '>
                  ...
                  <div className='flex'>
                    ...
                    }} className="my-2 mx-3  w-2/5 bg-white-1000 bg-opacity-40 shadow-md  text-base font-normal text-gray-500 font-bold bg-white bg-clip-padding bg-no-repeat border border-2 border-white-1000 rounded transition ease-in-out m-0 focus:text-gray-800 focus:bg-white focus:border-purple-600 focus:outline-none">
                      {
                        inputDevices.map(device => {
                          if (device.kind === "videoinput") {
                            return (
                              <option key={device.deviceId} value={device.deviceId} className='bg-sky-100 ' >{device.label}</option>
                            )
                          }
                        })
                      }
                    </select>
                    <select name="audioinput" onChange={(e) => setSelectedAudioInput(e.target.value)} className=" my-2 mx-3 w-2/5 bg-white-1000 bg-opacity-40 shadow-md  text-base font-normal text-gray-500 font-bold bg-white bg-clip-padding bg-no-repeat border border-2 border-white-1000 rounded transition ease-in-out m-0 focus:text-gray-800 focus:bg-white focus:border-purple-600 focus:outline-none">
                      {
                        inputDevices.map(device => {
                ...
                <div className='w-full h-[calc(100%-5.5rem)] '>
                  ...
                  <div className="h-[12%] grid place-items-center">
                    <div className="flex w-48 justify-between items-center">
                      <div className='w-12 h-12 rounded-xl flex justify-center bg-red-700 bg-opacity-80' onClick={() => exit()}>
                        <PhoneMissed className='m-auto text-indigo-100' />
                      </div>
                      {
                        producerDevices.includes(mediaType.screen) ?
                          <div className='w-10 h-10 rounded-xl flex justify-center bg-black bg-opacity-40' onClick={() => closeProducer(mediaType.screen)}>
                            {/* <Share className='m-auto text-indigo-100'/> */}
                            <img src={ShareOff} alt="" className='m-auto' />
                          </div> :
                          <div className='w-10 h-10 rounded-xl flex justify-center bg-black bg-opacity-40' onClick={() => produce(mediaType.screen)}>
                            <Share className='m-auto text-indigo-100' />
                          </div>
                      }
                      {
                        producerDevices.includes(mediaType.video) ?
                          <div className='w-10 h-10 rounded-xl flex justify-center bg-black bg-opacity-40' onClick={() => closeProducer(mediaType.video)}>
                            <VideoOff className='m-auto text-indigo-100' />
                          </div> :
                          <div className='w-10 h-10 rounded-xl flex justify-center bg-black bg-opacity-40' onClick={() => produce(mediaType.video, selectedVideoInput)}>
                            {/* <span>VRT</span> */}
                            <Video className='m-auto text-indigo-100' />
                          </div>
                      }

                      {
                        producerDevices.includes(mediaType.audio) ?
                          <div className='w-10 h-10 rounded-xl flex justify-center bg-black bg-opacity-40' onClick={() => closeProducer(mediaType.audio)}>
                            <MicOff className='m-auto text-indigo-100' />
                          </div> :
                          <div className='w-10 h-10 rounded-xl flex justify-center bg-black bg-opacity-40' onClick={() => { console.log("selecteAudioInput : ", selectedAudioInput); produce(mediaType.audio, selectedAudioInput) }}>
                            <Mic className='m-auto text-indigo-100' />
                          </div>
                      }
                    </div>
                  </div>
              ...
              <div className='w-[30rem] hidden md:block h-screen bg-slate-100 bg-opacity-50 '>
                <div className='flex justify-around align-middle h-12 mx-2 shadow-[0_35px_60px_-15px_rgba(0,0,0,0.3)]'>
                  ...
                  <h4 className={`${isMessageShown ? 'font-mono my-auto text-white-1000  rounded-md py-1 px-2 bg-gray-600' : 'font-mono my-auto'} `} onClick={() => setIsMessageShown(true)} >Messages</h4>
                  <h4 className={`${!isMessageShown ? 'font-mono my-auto text-white-1000  rounded-md py-1 px-2 bg-gray-600' : 'font-mono my-auto'} `} onClick={() => setIsMessageShown(false)} >Participants</h4>
                </div>
                <div className={`${!isMessageShown && 'hidden'} h-[calc(100%-7rem)] w-full overflow-y-scroll`} >
                  <h3 className='text-gray-500'>This chat will only continue while you are participating.</h3>
                  {
                    chats?.map(chat => {
                ...
                <div className={`${!isMessageShown && 'hidden'} h-12`}>
                ...
                <div className={`${isMessageShown && 'hidden'} h-[calc(100%-7rem)] w-full overflow-y-scroll px-3`} >
                  {
                    participants.map(participant => {
                      return (
                        <div className='bg-white-1000 bg-opacity-50 my-1 h-12 flex rounded-lg px-3'>
                          <h5 className='my-auto font-semibold' >{participant}</h5>
                        </div>
                      )
                    })
                  }
    ...
    return (
      <div className="flex mb-6 flex-row-reverse pr-3">
        <div className=' relative'>
          ...
          <div className='flex '>
            <div className='bg-sky-50 rounded-t-2xl rounded-bl-2xl px-3 py-2 mr-2  '>
              {chat.message}
            </div>
      ...
      <div className="flex mb-6 pl-3">
        <div className=' relative'>
          ...
          <div className='flex flex-row-reverse'>
            <div className='bg-slate-bg-sky-50 rounded-t-2xl rounded-br-2xl px-3 py-2 ml-2  '>
              {chat.message}
            </div>
            <div className='w-9 h-9 mt-auto rounded-full bg-slate-700'></div>
          </div>
...
// head = {value : 20, next : {value : 30, next : { value : 40 , next : null}}}

// tail = {value : 50, next : null}


// {value : 20, next : {value : 30, next : { value : 40 , next : {value : 50, next : null}}}}
